...
u32int kmalloc_int(u32int sz, int align, u32int *phys)
{
    if (kheap != 0)
    {
        ...
        if (phys != 0)
        {
            ...
            *phys = page->frame*0x1000 + ((u32int)addr&0xFFFFF000);
        }
        return (u32int)addr;
    }
    else
...
void *alloc(u32int size, u8int page_align, heap_t *heap)
{
    ...
    // Find the smallest hole that will fit.
    s32int siterator = find_smallest_hole(new_size, page_align, heap);

    if (siterator == -1) // If we didn't find a suitable hole
    {
    ...
    header_t *orig_hole_header = (header_t *)lookup_ordered_array(siterator, &heap->index);
    ...
    else
    {
        ...
        remove_ordered_array(siterator, &heap->index);
    }
